...
1. **Slide spec** → `investment-memo-{slug}-v{round}.md` — content brief: per-slide impact + facts.
...
The spec is a content brief: _what impact each slide must land and what facts go on it_. Each slide is a `## S{N}: {title}` block with a `<!-- slide -->` metadata comment, followed by **Impact**, **Content**, and (optional) **Notes**.

**Impact is the per-slide takeaway, not the topic** — one sentence stating what the slide must make the IC feel or conclude. It is the slide-specific version of the matching Content Catalog **Impact** in `assets/guide.md`.
...
**Impact:** Labor supply is shrinking while demand for fulfillment accelerates — the squeeze is the "why now."
...
`slide` metadata is one of `S1`–`S6`. Fixed slides (`S1`, `S2`, `S3`, `S5`, `S6`) are cloned from `reference.pptx` with text swapped. `S4` is a body slide — the renderer builds it from a blank canvas, designing the composition that lands the impact.
...
1. **Read the memo fully.** Match each section to a slide; decide its per-slide Impact.
...
3. **Sanity-check the spec** against guide.md's constraints and ship states. Impact sentences are takeaways, not topics.
